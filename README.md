...
3. 以后直接双击 WorkDaddy 图标即可；请勿以管理员身份运行
...
- **诊断遥测默认关闭**：只有显式设置 `WORKDADDY_TELEMETRY=1` 时，才会向 Sentry 发送经过脱敏的错误诊断；未启用时既不发送，也不刷新既有遥测队列。
- **完整渲染器日志默认关闭**：默认不持久记录 WorkBuddy 的 console 和异常正文；仅在本地排障时显式设置 `WORKDADDY_DIAGNOSTIC_LOGS=1` 才会写入经过脱敏、截断的诊断行，此开关不会启用 Sentry 遥测。
