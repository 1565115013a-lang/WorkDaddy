...
- **暂存提示词**：输入框边上一键把草稿「暂存」到待发送队列——图片 / 文件 / 引用原样保留，不会误发，想好了再发。
- **切换精美主题**：内置多套官方主题（含 Nebula 星云），支持自定义配色与背景图。
- **账号间会话迁移**：把 A 账号的整段历史会话一键复制到 B 账号，跨账号继续接龙。
- **强化决策弹窗**：WorkBuddy 让你选/确认时，弹窗来点而不是文本询问（全局自定义指令实现）。
- **防止电脑休眠**：三档休眠策略（允许 / 持续禁止 / 任务结束恢复），不让 AI 跑一半被黑屏掐断。
- **自动更新**：发现新版本自动下载安装，无需手动下载，更新完自动重启。
...
3. 以后只需**右键已管理员身份打开桌面的 WorkDaddy 图标** 即可启动
...
| Tab      | 能做什么                                                |
| -------- | ------------------------------------------------------- |
| **账号** | 列出所有已备份账号（昵称 / 手机 / 积分 / Token 过期）   |
