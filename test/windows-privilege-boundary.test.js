...
test('Windows entry points fail closed and expose a standard daemon identity', () => {
  ...
  assert.match(watchdogSource, /if \(state\.kind === 'stale'\) throw new Error\(/);
  assert.doesNotMatch(watchdogSource, /state\.kind === 'stale'\) removePidFileIf/);
  assert.match(launcherSource, /if \(watchdog\.kind === 'stale'\) throw new Error\(/);
  assert.doesNotMatch(launcherSource, /watchdog\.kind === 'stale'[\s\S]{0,100}(?:unlinkSync|removePidFileIf)/);
